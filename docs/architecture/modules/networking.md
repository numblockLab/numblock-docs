...
To accomplish this task, the NumBlock leverages the battle-tested **libp2p** framework.
...
* **Modularity** - libp2p is modular by nature, just like the NumBlock. This gives greater flexibility, especially when parts of the NumBlock need to be swappable

## GRPC

On top of **libp2p**, the NumBlock uses the **GRPC** protocol. <br />
Technically, the NumBlock uses several GRPC protocols, which will be covered later on.

The GRPC layer helps abstract all the request/reply protocols and simplifies the streaming protocols needed for the NumBlock to function.
...
Earlier, we mentioned that the NumBlock leverages several GRPC protocols.<br />
...
The following section might seem familiar because it was briefly covered in the [CLI Commands](/docs/validators/cli-commands) section.
...
The NumBlock also implements several service methods that are used by other nodes on the network. <br />
