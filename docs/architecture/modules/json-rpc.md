...
The **REST API** module implements the **JSON RPC API layer**, something that dApp developers use to interact with the NumBlock
blockchain.
...
The NumBlock uses the ***blockchain interface*** to define all the methods that the REST API module needs to use, in
order to deliver its endpoints.

It is the base implementation that's passed into the REST API layer.
