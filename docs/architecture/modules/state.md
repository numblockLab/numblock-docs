...
To truly understand how **State** works, you must understand some basic NumBlock concepts.<br />

We highly recommend reading the **[State in NumBlock guide](/docs/foundational-topics/state)**.

## Overview

Now that we've familiarized ourselves with basic NumBlock concepts, the next overview should be easy.

We mentioned that the **World state trie** has all the NumBlock accounts that exist. <br />
These accounts are the leaves of the Merkle trie. Each leaf has encoded **Account State** information.

This enables the NumBlock to get a specific Merkle trie, for a specific point in time. <br />
...
*state/executor.go* includes all the information needed for the NumBlock to decide how a block changes the current
