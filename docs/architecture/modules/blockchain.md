...
One of the main modules of the NumBlock are **Block Management** and **State**. <br />

**Block Management* is the powerhouse that deals with block reorganizations. This means that it deals with all the logic that happens when a new block is included in the blockchain.
...
The key takeaway is that these 2 parts are very connected, and they work closely together in order for the client to function. <br /> For example, when the **Block Management** layer receives a new block (and no reorganization occurred), it calls the **State** to perform a state transition.

**Management** also has to deal with some parts relating to consensus (ex. *is this ethHash correct?*, *is this PoS correct?*). <br /> In one sentence, **it is the main core of logic through which all blocks are included**.

## *WriteBlocks*

One of the most important parts relating to the **Block Management** layer is the *WriteBlocks* method:
...
:::tip Refresher
Do you remember when we mentioned the ***monitor*** command in the [CLI Commands](/docs/validators/cli-commands)?

The Blockchain Events are the original events that happen in NumBlock, and they're later mapped to a Protocol Buffers message format for easy transfer.
:::