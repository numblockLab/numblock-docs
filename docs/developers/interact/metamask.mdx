--- 
description: Detecting that MetaMask is installed and adding additional networks and custom tokens easily make a great dApp user experience, specially those who are not technically skilled. It is important to streamline the onboarding process as much as you can to make it easier for consumers to operate your application. This guide explains how to build a simple button for your front-end application that will automatically add Figchain to MetaMask.
keywords: [metamask, metamask api, detect metamask, detect chainid, detect account]
---

# MetaMask API

Detecting that MetaMask is installed and adding additional networks and custom tokens easily make a great dApp user experience, specially those who are not technically skilled. It is important to streamline the onboarding process as much as you can to make it easier for consumers to operate your application. This guide explains how to build a simple button for your front-end application that will automatically add NumBlock to MetaMask.

## Summary of actions

1. Detect MetaMask or propose its installation
2. Detect ChainId, and propose to add NumBlock if needed
...
The following code includes all the parameters needed by MetaMask to add NumBlock to its networks programmatically

```js showLineNumbers
var NUMBLOCK_MAINNET_PARAMS = {
    chainId: "0x64",
    chainName: "numblock",
    nativeCurrency: {
        name: "NUMB",
...
const tokenAddress = 'custom-token-address-on-numblock';
