...
Tokens can represent virtually anything in NumBlock:
...
Such a powerful feature of NumBlock must be handled by a robust standard, right? That's exactly
where the NRC-20 plays its role! This standard allows developers to build token applications that are interoperable with other products and services.

**What is NRC-20?**

The NRC-20 introduces a standard for Fungible Tokens, in other words, they have a property that makes each Token be exactly
...
Example functionalities NRC-20 provides:
...
If a Smart Contract implements the following methods and events it can be called an NRC-20 Token Contract and, once deployed, it
will be responsible to keep track of the created tokens on NumBlock.
...
Let's see how a Standard is so important to make things simple for us to inspect any NRC-20 Token Contract on NumBlock.
...
- [OpenZeppelin - NRC-20 Implementation](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/ERC20.sol)
