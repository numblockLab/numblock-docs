...
description: Tutorial to deploy an NFT collection to NumBlock.
keywords: [nft contract, deploy nft, erc721, erc721 standard, erc721 token]
---

# Launching an NFT on NumBlock

## Overview

As is the case with many other things on NumBlock, launching an NFT collection follows very similar steps to how you would on NumBlock. As is the case with NumBlock, you will need to implement the [ERC721 standard](https://eips.ethereum.org/EIPS/eip-721) to create a Non-Fungible Token. For those familiar with Object-Oriented Programming, it's much like implementing an interface. You need to implement each of the following events/functions:
...
:::note
NFTs are not necessarily NRC-721 tokens, they can also be [NRC-1155](https://eips.ethereum.org/EIPS/eip-1155), for example.
:::
...
For this walk through, we're going to be using [Hardhat](https://hardhat.org/) ([configure it with NumBlock](/docs/developers/smart-contracts/deploying/hardhat)).

## Prerequisites 
To follow along, it's recommended to review and be familiar with the [documentation on deploying a contract](/docs/developers/building/first-contract).
You will also need to have a working Node.js >=16.0 installation and [a small amount of xDai for gas](/docs/foundational-topics/networks/explorers/faucet).
...
## Step 3: Implement the NRC-721 Token Contract

1. Create a new directory called contracts and create a file called NumBlockNft.sol
...
To keep it simple for the sake of this tutorial, we're going to import OpenZeppelin's [NRC-721 Implementation](https://github.com/OpenZeppelin/openzeppelin-contracts/tree/master/contracts/token/ERC721). To use this, quickly run in the terminal:
...
```
3. Now that you've got that all coded up, it's time to compile and deploy. You can also [see here](/docs/developers/smart-contracts/deploying/hardhat) for more deployment info. Run from project root:
```bash
...
To deploy, run:
```bash
npx hardhat run scripts/deploy.js --network numblock
```
Congrats, you have deployed a basic NRC-721 contract to NumBlock! If you like, you can build out a front end to view your NFT. For now, you can view your token in [NumBlock Testnet](https://testnet.numblock.org/). 
